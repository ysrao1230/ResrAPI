...
public class sample {
	...
	public static void main(String[] args) {
		...
		/*
		 * String str = null; System.out.println(String.valueOf(str)); // This will
		 * print a String equal to "null" System.out.println(str.toString()); // This
	...
	public void test_delete() {
		...
		for (int i = 0; i <= 2; i++) {
			...
			JSONObject request = new JSONObject();
			given().body(request.toJSONString()).pathParam("value", i).when()
					.delete("https://reqres.in/api/users/{value}").then().statusCode(204).log().all();
	...
	public void test_ResponseHeaderData_ShouldBeCorrect() {
		...
		given().when().get("http://ergast.com/api/f1/2017/circuits.json").then().assertThat().statusCode(200).and()
				.contentType(ContentType.JSON).and().header("Content-Length", equalTo("4551")).log().all();
	}
	...
	public void test_NumberOfCircuits_ShouldBe20_Parameterized(String series, String season, int numberOfRaces) {
		given().pathParam("value", series).pathParam("test", season).when()
				.get("http://ergast.com/api/{value}/{test}/circuits.json").then().assertThat()
				.body("MRData.CircuitTable.Circuits.Location", hasSize(numberOfRaces)).log().all();
	}

	@DataProvider(name = "seasonsAndNumberOfRaces")
	public Object[][] createTestDataRecords() {
		return new Object[][] { { "f1", "2017", 20 }, { "f1", "2016", 21 }, { "f1", "1966", 9 }, { "f1", "2010", 19 } };
	}
	...
	public void getResponseBody() {
		...
		int status_code = given().queryParam("CUSTOMER_ID", "68195").queryParam("PASSWORD", "1234!")
